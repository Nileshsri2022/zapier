...
const WhatsAppServerConfig = () => {
    ...
    const fetchWhatsAppServers = async () => {
        try {
            ...
            toast.error('Failed to fetch WhatsApp servers: ' + (error.response?.data?.message || error.message));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const createWhatsAppServer = async () => {
        if (!formData.displayName.trim() || !formData.phoneNumberId.trim() || !formData.accessToken.trim()) {
        ...
        try {
            ...
            setFormData({ displayName: '', phoneNumberId: '', businessId: '', accessToken: '', phoneNumber: '' });
            fetchWhatsAppServers();
        } catch (error: any) {
            toast.error('Failed to connect WhatsApp: ' + (error.response?.data?.message || error.message));
        }
    };
    ...
    const testConnection = async (serverId: string) => {
        try {
            ...
            await axios.post(`${API_URL}/api/whatsapp/servers/${serverId}/test`, {}, {
                headers: { Authorization: token }
            });
    ...
    const toggleStatus = async (serverId: string, isActive: boolean) => {
        try {
            ...
            await axios.put(`${API_URL}/api/whatsapp/servers/${serverId}`, { isActive: !isActive }, {
                headers: { Authorization: token }
            });
                ...
                <div className="bg-white p-6 rounded-lg shadow-md mb-6">
                    ...
                    <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                        ...
                        <ol className="text-sm text-blue-700 list-decimal list-inside space-y-1">
                            <li>Go to <a href="https://developers.facebook.com/apps" target="_blank" className="underline">Meta Developers</a></li>
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">
                                        Phone: {server.phoneNumber || 'Not set'}
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        Created: {new Date(server.createdAt).toLocaleDateString()}
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        Triggers: {server.triggers?.length || 0}
                                    </p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className={`px-2 py-1 text-xs rounded-full ${server.webhookVerified
