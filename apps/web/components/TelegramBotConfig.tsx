...
const TelegramBotConfig = () => {
    ...
    const fetchBots = async () => {
        try {
            ...
            toast.error('Failed to fetch Telegram bots: ' + (error.response?.data?.message || error.message));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const testConnection = async (botId: string) => {
        try {
            ...
            const response = await axios.post(`${API_URL}/api/telegram/bots/${botId}/test`, {}, {
                headers: { Authorization: token }
            });
    ...
    const toggleStatus = async (botId: string, isActive: boolean) => {
        try {
            ...
            await axios.put(`${API_URL}/api/telegram/bots/${botId}`, { isActive: !isActive }, {
                headers: { Authorization: token }
            });
                        ...
                        <div key={bot.id} className="bg-white p-6 rounded-lg shadow-md">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">
                                        Triggers: {bot.triggers?.length || 0}
                                    </p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className={`px-2 py-1 text-xs rounded-full ${bot.webhookConfigured
                                            ? 'bg-blue-100 text-blue-800'
                                            : 'bg-yellow-100 text-yellow-800'
                                        }`}>
                                        {bot.webhookConfigured ? 'Webhook ✓' : 'Webhook Pending'}
                                    </span>
                                    <span className={`px-2 py-1 text-xs rounded-full ${bot.isActive
