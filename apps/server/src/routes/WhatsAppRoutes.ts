...
router.post('/servers', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    try {
        // @ts-ignore
        const userId = req.id;
        if (!userId) {
        ...
        if (!displayName || !phoneNumberId || !accessToken) {
            return res.status(400).json({ message: 'Display name, phone number ID, and access token are required' });
        }
...
router.post('/servers/:id/test', authMiddleware, async (req: Request, res: Response): Promise<any> => {
    try {
        // @ts-ignore
        const userId = req.id;
        const { id } = req.params;
        ...
        const response = await axios.get(
            `https://graph.facebook.com/v18.0/${server.phoneNumberId}`,
            {
