...
router.post('/', authMiddleware, async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const validTypes = ['minutely', 'hourly', 'daily', 'weekly', 'monthly'];
    ...
    console.log(
      `📅 Schedule created/updated for Zap "${zap.name}" - Next run: ${nextRunAt.toISOString()}`
    );
