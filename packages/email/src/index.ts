 ...
 */
async function checkRecipientCooldown(email: string): Promise<{ allowed: boolean; cooldownRemaining: number }> {
    try {
...
export const sendEmail = async (to: string, subject: string, templateName: string, templateData?: object) => {
    try {
        ...
        console.log(`✅ Email sent successfully: ${data?.id} (${rateLimit.remaining} remaining this hour)`);
        return data;
    } catch (error) {
